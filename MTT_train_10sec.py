...
def create_mtt_track_embeddings_no_padding():
    """
    MTT 데이터셋의 훈련 오디오 파일을 10초 세그먼트로 나누고 (패딩 없음),
    MuQ 모델의 가변 길이 시퀀스 임베딩을 그대로 저장합니다. (메모리 사용량 최적화 및 데이터 유효성 검사 추가)
    """
    # --- 경로 설정 ---
    # 1. MTT 데이터셋이 포함된 MARBLE 폴더 경로를 지정해주세요.
    AUDIO_BASE_PATH = '/home/jay/MusicAI/MARBLE'

    # 2. 'MTT.train.jsonl' 파일의 경로를 지정해주세요.
    JSONL_FILE_PATH = '/home/jay/MusicAI/MARBLE/data/MTT/MTT.train.jsonl'

    # 3. 생성된 임베딩 벡터를 저장할 폴더 경로를 지정해주세요.
    OUTPUT_EMBEDDINGS_PATH = 'mtt_embeddings_train_60sec'
    # -----------------
    ...
    for metadata in tqdm(train_files_metadata, desc="오디오 파일 처리 중"):
        relative_path = ""
        try:
            # JSONL 파일에서 상대 경로를 가져옵니다 (예: 'data/MTT/mp3/...')
            relative_path = metadata['audio_path']
            # 기본 경로와 합쳐 전체 파일 경로를 만듭니다.
...
if __name__ == '__main__':
    create_mtt_track_embeddings_no_padding()
