...
def create_gtzan_track_embeddings_no_padding():
    """
    GTZAN 데이터셋의 훈련 오디오 파일을 10초 세그먼트로 나누고 (패딩 없음),
    MuQ 모델의 가변 길이 시퀀스 임베딩을 그대로 저장합니다. (Windows 호환)
    """
    # --- 경로 설정 ---
    # 1. GTZAN 데이터셋의 최상위 폴더 경로를 지정해주세요.
    AUDIO_BASE_PATH = '/home/jay/MusicAI/MARBLE/data/GTZAN' # Windows 예시: r'C:\Users\user\MARBLE\GTZAN'

    # 2. 'GTZANGenre.train.jsonl' 파일의 경로를 지정해주세요.
    JSONL_FILE_PATH = '/home/jay/MusicAI/MARBLE/data/GTZAN/GTZANGenre.train.jsonl' # Windows 예시: r'C:\Users\user\MARBLE\GTZAN\GTZANGenre.train.jsonl'

    # 3. 생성된 임베딩 벡터를 저장할 폴더 경로를 지정해주세요.
    OUTPUT_EMBEDDINGS_PATH = 'gtzan_embeddings_train_60sec'
    # -----------------
    ...
    for metadata in tqdm(train_files_metadata, desc="오디오 파일 처리 중"):
        ...
        try:
            ...
            # === 수정 시작: GTZAN 경로 구조에 맞게 수정 ===
            # JSONL의 경로 "data/GTZAN/genres/..." 에서 "data/GTZAN/" 부분을 제거
            if relative_path_from_json.startswith('data/GTZAN/'):
                relative_path_from_json = relative_path_from_json[len('data/GTZAN/'):]
...
if __name__ == '__main__':
    create_gtzan_track_embeddings_no_padding()
