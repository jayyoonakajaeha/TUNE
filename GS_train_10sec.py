...
def create_gs_track_embeddings_no_padding():
    """
    GS 데이터셋의 훈련 오디오 파일을 10초 세그먼트로 나누고 (패딩 없음),
    MuQ 모델의 가변 길이 시퀀스 임베딩을 그대로 저장합니다. (메모리 사용량 최적화 및 데이터 유효성 검사 추가)
    """
    # --- 경로 설정 ---
    # === 수정 시작: 실제 파일 경로에 맞게 기본 경로 변경 ===
    # 1. GS 데이터셋의 최상위 폴더 경로를 지정해주세요.
    AUDIO_BASE_PATH = '/home/jay/MusicAI/MARBLE/data/GS'
    # === 수정 끝 ===

    # 2. 'GS.train.jsonl' 파일의 경로를 지정해주세요.
    JSONL_FILE_PATH = '/home/jay/MusicAI/MARBLE/data/GS/GS.train.jsonl'

    # 3. 생성된 임베딩 벡터를 저장할 폴더 경로를 지정해주세요.
    OUTPUT_EMBEDDINGS_PATH = 'gs_embeddings_train_10sec'
    # -----------------
    ...
    for metadata in tqdm(train_files_metadata, desc="오디오 파일 처리 중"):
        ...
        try:
            ...
            # JSONL의 경로 "data/GS/giantsteps_clips/..." 에서 "data/GS/" 부분을 제거
            if relative_path_from_json.startswith('data/GS/'):
                relative_path = relative_path_from_json[len('data/GS/'):]
            else:
                relative_path = relative_path_from_json
...
if __name__ == '__main__':
    create_gs_track_embeddings_no_padding()
